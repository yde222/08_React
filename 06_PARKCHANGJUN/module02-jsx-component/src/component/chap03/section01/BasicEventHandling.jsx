...
// use client를 명싲벅으로 선언하지 않으면 서버 컴포넌트로 동작, 선언하면 클라이언트 컴포넌트로 동작을 진행한다.
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: "20px" }}>
        <h3>HTML vs React 이벹느 처리비교</h3>
        <pre>
          {`
          html 방식: <button onclick="handleClick()>...
          React 방식: <button onClick={handleClick}>...</button>
          `}
        </pre>
      </div>
      {/* 잘못된 이벤트 핸들링 예시 */}
      {/*<button onClick={handleCilck()}>클릭2</button>*/}
      {/* 올바른 방식 (함수 참조를 전달 ) */}
      <button onClick={handleClick}>클릭2</button>
      <p>
        잘못된 방식에서는 렌더링 시점에 함수가 즉시 실행되며, 이벤트 핸들로
        undefined가 할당된다.
      </p>
    </div>
  );
